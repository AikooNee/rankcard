...
const RankCard = async (options) => {
    ...
    const cr = options.shape === "circle" ? av.width / 2 : 45;

    avCtx.beginPath();
    avCtx.moveTo(0 + cr, 0);
    avCtx.arcTo(av.width, 0, av.width, av.height, cr);
    avCtx.arcTo(av.width, av.height, 0, av.height, cr);
    avCtx.arcTo(0, av.height, 0, 0, cr);
    avCtx.arcTo(0, 0, av.width, 0, cr);
    avCtx.closePath();
    avCtx.clip();
